...
module.exports = {
    async list(ctx, next) {
        ...
        try {
            let data = await ctx.find(topicModel,null,null,{limit: count, skip: (start-1)*count, sort: {level: -1, createTime: -1}});
            console.log(data)
    ...
    async topicsByKey(ctx, next) {
        ...
        try {
            let data = await ctx.find(topicModel,{"title" : {$regex:title}},null,{limit: count, skip: (start-1)*count, sort: {level: -1, createTime: -1}});
            return ctx.send(data);
        } catch (e) {
            return ctx.sendError(e)
        }
