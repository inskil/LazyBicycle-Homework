    ...
    <div>
        <div>
            <Menu active-name="all" style="float: left; width: 15%">
                <MenuItem @click.native="filter('all')" name="all" class="search-menu">
                    <Icon type="ios-paper"/>
                    全部
                </MenuItem>
                <MenuItem @click.native="filter('book')" name="book" class="search-menu">
                    <Icon type="ios-book"/>
                    书籍
                </MenuItem>
                <MenuItem @click.native="filter('movie')" name="movie" class="search-menu">
                    <Icon type="ios-film"/>
                    影视
                </MenuItem>
                <MenuItem @click.native="filter('group')" name="group" class="search-menu">
                    <Icon type="ios-people"/>
                    小组
                </MenuItem>
                <MenuItem  @click.native="filter('topic')" name="discussion" class="search-menu">
                    <Icon type="ios-text"/>
                    讨论
                </MenuItem>
            </Menu>
            <table style="left: 20%; width:70%; position: absolute; font-size: larger">
                <tr style="margin-top: 10%"
                    v-if="(alllist.book.length+alllist.group.length+alllist.movie.length+alllist.topic.length)===0">
                    暂无相关内容~请上传或者更换关键字搜索哟~
                </tr>
                <tr v-for="book in alllist.book" v-bind:key="book._id" style="padding: 1rem">
                    <th align="left"
                        ...
                        <div>
                            ...
                            <Rate disabled allow-half v-model="book.rating[0].star"
                                  style="bottom: 0.5rem; position: relative"/>
                        </div>
        ...
        methods: {
            getlist() {
                ...
                axios.get("/search", pa).then(function (result) {
                    ...
                    for (var i = 0; i < _this.list.book.length; i++) {
                        _this.list.book[i].rating[0].star = _this.list.book[i].rating[0].average / 2;
                    }
                    for (i = 0; i < _this.list.movie.length; i++) {
                        _this.list.movie[i].rating[0].star = _this.list.movie[i].rating[0].average / 2;
                    }
            ...
            filter(flag) {
                console.log("ffffffffffffffffffffffffffff",flag)
                if (flag == 'all') {
                    ...
                    this.alllist[flag] = this.list[flag]
                }
                console.log("ffffalllistfffff",this.alllist)
            }
        },
        created() {
            this.getlist();
